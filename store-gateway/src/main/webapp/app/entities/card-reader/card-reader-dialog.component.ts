...
import { Acquirer, AcquirerService } from '../acquirer';
...
export class CardReaderDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.acquirerService.query()
            .subscribe((res: HttpResponse<Acquirer[]>) => { this.acquirers = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackAcquirerById(index: number, item: Acquirer) {
        return item.id;
    }
}
