...
import { IssuingBank, IssuingBankService } from '../issuing-bank';
...
export class DebitDialogComponent implements OnInit {
    ...
    ngOnInit() {
        this.isSaving = false;
        this.issuingBankService.query()
            .subscribe((res: HttpResponse<IssuingBank[]>) => { this.issuingbanks = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackIssuingBankById(index: number, item: IssuingBank) {
        return item.id;
    }
}
