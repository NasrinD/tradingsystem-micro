...
describe('CardReader e2e test', () => {
    ...
    it('should create and save CardReaders', () => {
        ...
        cardReaderDialogPage.setCashDeskidInput('5');
        expect(cardReaderDialogPage.getCashDeskidInput()).toMatch('5');
        cardReaderDialogPage.controllerSelectLastOption();
        cardReaderDialogPage.acquirerSelectLastOption();
...
export class CardReaderDialogPage {
    ...
    cashDeskidInput = element(by.css('input#field_cashDeskid'));
    controllerSelect = element(by.css('select#field_controller'));
    acquirerSelect = element(by.css('select#field_acquirer'));
    ...
    setCashDeskidInput = function(cashDeskid) {
        this.cashDeskidInput.sendKeys(cashDeskid);
    };

    getCashDeskidInput = function() {
        return this.cashDeskidInput.getAttribute('value');
    };
    ...
    acquirerSelectLastOption = function() {
        this.acquirerSelect.all(by.tagName('option')).last().click();
    };

    acquirerSelectOption = function(option) {
        this.acquirerSelect.sendKeys(option);
    };

    getAcquirerSelect = function() {
        return this.acquirerSelect;
    };

    getAcquirerSelectedOption = function() {
        return this.acquirerSelect.element(by.css('option:checked')).getText();
    };
