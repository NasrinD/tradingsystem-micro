...
import org.jhipster.bankservice.repository.DebitRepository;
...
public class DebitResource {
    ...
    @PostMapping("/debits")
    @Timed
    public ResponseEntity<Debit> createDebit(@Valid @RequestBody Debit debit) throws URISyntaxException {
    ...
    @PutMapping("/debits")
    @Timed
    public ResponseEntity<Debit> updateDebit(@Valid @RequestBody Debit debit) throws URISyntaxException {
        log.debug("REST request to update Debit : {}", debit);
        if (debit.getId() == null) {
            return createDebit(debit);
        }
        Debit result = debitRepository.save(debit);
    ...
    public ResponseEntity<Debit> getDebit(@PathVariable Long id) {
        ...
        Debit debit = debitRepository.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(debit));
    }
